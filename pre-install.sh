...
ssh root@$ip_2 -p $port_2 "ip=\$(wget -qO- eth0.me) && cd ~ && chmod +x openvpn-install.sh && sed -i '43,76d;588,593d' openvpn-install.sh && sed -i 's/"redirect-gateway def1 bypass-dhcp"/route $ip 255.255.255.0/g' openvpn-install.sh && sed -i 's/10.8.8./10.8.9./g' openvpn-install.sh"
