...
if [ "$(ssh root@$ip_2 -p $port_2 ls -l | grep -o shadowsocks.sh)" = "shadowsocks.sh" ] && [ "$(ssh root@$ip_2 -p $port_2 cat shadowsocks.sh | grep -o "RED")" = "RED" ];then break;else ssh root@$ip_2 -p $port_2 rm -f shadowsocks.sh; fi
done
